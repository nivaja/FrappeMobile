...
import 'pagination_box.dart';
import 'table_column.dart';

typedef TableHeaderBuilder = Widget Function(String? header, int index);
typedef TableCellBuilder = Widget Function(dynamic value, List<int> index);
...
class _JsonTableState extends State<JsonTable> {
          ...
          SingleChildScrollView(
            ...
            child: (widget.columns != null)
              ...
              children: widget.columns!
                  ...
                  .entries
                  .where((item) =>
                  filterHeaderList.contains(item.value.field))
                  .map(
                    (item) => TableColumn(
                  item.value.label,
                  item.key,
  ...
  onRowHold(int index) {
    setState(() {
      var idx = selectedRowIndexes.indexOf(index);
      if (idx != -1) {
        selectedRowIndexes.removeAt(idx);
      } else {
